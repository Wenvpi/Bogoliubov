...
If we work with the $\zeta_1$'s 4th order ODE, we would need the initial conditions for $\zeta_1$ and its derivatives. A simple asymptotic analysis reveal that the linearly independent solutions $\zeta_{1,j}$, $j=1,2,3,4$,  have the following oscillatory behavior when $x\to +\infty$:
\[
\begin{aligned}
...
\end{aligned}
\]
Here, l.o.t. means terms decaying to zero as $x\to +\infty$, as fast as $x^{-1}$ in absolute value with respect to the previous member of the expansion. Additionally, $\zeta_{1,1}(x,0)=\zeta_{1,2}(x,0)$. Let $\zeta_{1,0}(x,\lambda)$ be the {\bf scattering or Jost} solution of \eqref{eq:zeta_1(x)} such that 
\[
...
\end{aligned}
\]
Notice that $\zeta_{1,1}$ not not be equal to $\zeta_{1,1}^0$ and so on. The purpose next is to find $\zeta_{1,1}$ and $\zeta_{1,2}$, which will be related to $\zeta_{1,2}^0$.
...
& \quad =  -i e^{\frac{\pi  \lambda}{2}}  \left( (1-\lambda i)  L_{i \lambda/2}(-2 i x)  + i\lambda \, _1F_1\left(1- \frac{\lambda}2 i ;2;-2 i x \right) \right),
\end{aligned}}
\]
%Luego,
%\[
...
\boxed{\begin{aligned}
& \mathcal L^{-1}\left(\widetilde L_{1,-\lambda} (s) \right)(x)\\
& \quad = -i  e^{-\frac{\pi  \lambda}{2}}   \left( (1+ \lambda i)L_{-i \lambda/2}(-2 i x) - i\lambda \, _1F_1\left(1+  \frac{\lambda}2i ;2;-2 i x \right) \right).
\end{aligned}}
\]
...
\end{aligned}
\end{equation}
Assume $x>0$. We get that if $f_0(\lambda):= \Gamma(1+\frac12i\lambda)  (1-\lambda i)^{-1}(2i)^{-i\lambda/2}$, then
...
\end{aligned}
\end{equation}
are linearly independent solutions of the ODE. We know that
\begin{equation*}
...
\end{equation*}
Since \eqref{cond_imp} holds, one needs to check that $-2i \zeta_1'(0^+) = \lambda^2 \zeta_1(0^+)$. This is indeed satisfied for $\zeta_{1,1}$ and $\zeta_{1,2}$.

We can prove that \eqref{eq:sol-zeta1(x)_xpos} are L.I. solutions replacing them in \eqref{eq:zeta_1(x)}. Doing this we obtain
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
that, apparently, is non-zero. It might be useful to consider the relation
\begin{equation*}
L_\nu (z) = \,_1F_1(-\nu;1;z)
\end{equation*}
so
...
\end{aligned}
\end{equation}
where due to $x>0$ we can ignore the $1/x$ term.

However, plotting \eqref{eq:ODE[zeta_11]}, FIG~\eqref{fig:ODE}, we can see just numerical noise, therefore we can conclude that actually \eqref{eq:sol-zeta1(x)_xpos} are solutions, but we, and \texttt{Mathematica}, are not able to simplify and reduce this expression.
\begin{figure}[h]
\includegraphics[width=0.8\linewidth]{img/ODE.pdf}
\caption{\label{fig:ODE} Plot of \eqref{eq:ODE[zeta_11]}. Independent of $x$ real and imaginary parts take random values, then we can related them with numerical noise.}
\end{figure}
...
Until now we have 2 solutions for our problem, but the 2 others are missing. In this section we will use the Fourier transformation, defined as
\begin{equation}
   F(k) := \int_{-\infty}^{\infty}\dd{x}f(x)e^{ikx}
\end{equation}
with its respective inverse
\begin{equation}
   f(x) = \int_{-\infty}^{\infty}\frac{\dd{k}}{2\pi}F(k)e^{-ikx},
\end{equation}
to try to find the missing solutions. Is important to mention that we are interested on the $\zeta_1(x)$ coefficient, that is defined in the range $x>0$. Therefore, we will extrapolate this coefficient to $x<0$ considering an \textbf{even copy} of the function in $x>0$.

Using the definitions mentioned we can prove that
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
We already know that $\zeta_{1,1}$ and $\zeta_{1,2}$ do not converge in $x=\pm \infty$, thus, in theory, we should consider a non-zero boundary term coming from $f(x)e^{ikx}\Big|_{-\infty}^{\infty}$. Nevertheless, we will focus on new solutions that actually tend to zero in $x=\pm \infty$. Thus let us just disregard boundary terms coming from integration by parts,
\begin{equation*}
   \Rightarrow \mathcal{F}\qty[f'(x)](k) = -ikF(k).
\end{equation*}
It is easy to extend this result for the Fourier transform of $f^{(n)}(x)$, with $n$ arbitrary, that would be
\begin{equation}
   \mathcal{F}\qty[f^{(n)}(x)] = \qty(-ik)^nF(k).
\end{equation}
Also we need the Fourier transform of $x$ powers by derivatives of $f(x)$,
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
Finally, using these relations and assuming
\begin{equation}
   \lim_{x\to\pm\infty}\qty(\zeta_1,\zeta_1',\zeta_1'',\zeta_1^{(3)},\zeta_1^{(4)}) = (0,0,0,0,0)
\end{equation}
we obtain a homogeneous 2nd order ODE
\begin{equation}
   \left(-k^4+4 k^3-4 k^2\right) F''(k)+\left(-4 k^3+14 k^2-12 k\right) F'(k)+F(k) \left(-\lambda ^2+2 k-4\right) = 0,
\end{equation}
where $F^{(j)}(k) \equiv \pdv*[j]{F(k)}{k}$, and has as solution
\begin{equation}
   F(k) = \frac{c_1 P_1^{i \lambda }(k-1)}{k}+\frac{c_2 Q_1^{i \lambda }(k-1)}{k}.
\end{equation}
Using \texttt{Mathematica} we obtain an expression, exactly as when we did it with Laplace transform, for $P_1^{i\lambda}$
\begin{equation}
    P_1^{i\lambda}(k - 1) = \frac{1}{\Gamma(2-i\lambda)}(2-k)^{-i\frac{\lambda}{2}}(-1-i\lambda + k)k^{i\frac{\lambda}{2}}
\end{equation}
and
\begin{equation}
    Q_1^{i\lambda}(k - 1) = f_1(\lambda)P_1^{-i\lambda}(k - 1) + g_1(\lambda)P_1^{i\lambda}(k - 1),
\end{equation}
with $f_1$ and $g_1$ defined in \eqref{eq:f_1 and g_1}. Thus, our solution will be expressed as
\begin{equation}
\begin{aligned}
    F(k) &= \tilde{f}_0(\lambda)(-1 + i\lambda + k)(2 - k)^{i\frac{\lambda}{2}}k^{-i\frac{\lambda}{2} - 1}\\
    &\quad + \tilde{g}_0(\lambda)(-1 - i\lambda + k)(2 - k)^{-i\frac{\lambda}{2}}k^{i\frac{\lambda}{2} - 1}\\
    &=: \tilde{f}_0(\lambda)F_{\lambda}(k) + \tilde{g}_0(\lambda)F_{-\lambda}(k), 
\end{aligned}
\end{equation}
where
\begin{equation}
    F_{\pm \lambda}(k) := (-1 \pm i\lambda + k)(2 - k)^{\pm i\frac{\lambda}{2}}k^{\mp i\frac{\lambda}{2} - 1}.
\end{equation}
Then, the only thing left is compute the inverse Fourier transform of $F(k)$.

% \subsection{Expanding around $\lambda$}
% Until now, we have not been able to compute the full inverse Fourier transform of $F(k)$. However, expanding around $\lambda = 0$ up to first order on this parameter
% \begin{equation*}
% F_{\pm \lambda}(k) = \frac{k-1}{k} \pm \frac{i \lambda}{2 k}  [(k-1) \log (2-k)-k \log (k)+\log (k)+2] + \mathcal{O}(\lambda^2)
% \end{equation*}
% where it inverse Fourier transform is given by
% \begin{equation*}
% \mathcal{F}^{-1}\qty[F_{\pm \lambda}(k)] = \pm\lambda\sqrt{\frac{\pi}{2}}\qty[ i\frac{1}{x} - \log(x)] + \frac{1}{8} \sqrt{\frac{\pi }{2}} \qty[\mp \lambda  (-8+8 \gamma -4 i \pi +\log (256)) + 8 i] + \mathcal{O}(\lambda^2).
% \end{equation*}
% Let us call these functions as $\tilde{\zeta}_{1,3}$ and $\tilde{\zeta}_{1,4}$, explicitly
% \begin{equation}
% \begin{aligned}
% \tilde{\zeta}_{1,3}(x) &:= \lambda\sqrt{\frac{\pi}{2}}\qty[ i\frac{1}{x} - \log(x)] + \frac{1}{8} \sqrt{\frac{\pi }{2}} \qty[- \lambda  (-8+8 \gamma -4 i \pi +\log (256)) + 8 i] + \mathcal{O}(\lambda^2)\\
% \tilde{\zeta}_{1,4}(x) &:= -\lambda\sqrt{\frac{\pi}{2}}\qty[ i\frac{1}{x} - \log(x)] + \frac{1}{8} \sqrt{\frac{\pi }{2}} \qty[+ \lambda  (-8+8 \gamma -4 i \pi +\log (256)) + 8 i] + \mathcal{O}(\lambda^2).
% \end{aligned}
% \end{equation}
% To be consistent, we must also expand our previous solutions \eqref{eq:sol-zeta1(x)_xpos} up to first order on $\lambda$
% \begin{equation}
% \begin{aligned}
% \zeta_{1,1}(x) &= \frac{\lambda}{2}\frac{1 - e^{-2 i x}}{x} + \frac{ i \lambda}{2}  \pdv{\alpha}L_\alpha(-2 i x)\Big|_{\alpha = 0} + 1 -i \lambda + \mathcal{O}(\lambda^2)\\
% \zeta_{1,2}(x) &= -\frac{\lambda}{2}\frac{1 - e^{-2 i x}}{x} - \frac{ i \lambda}{2}  \pdv{\alpha}L_\alpha(-2 i x)\Big|_{\alpha = 0} + 1 + i \lambda + \mathcal{O}(\lambda^2).
% \end{aligned}
% \end{equation}
% Therefore, the final solution, up to order $\lambda$, is given by
% \begin{equation}
% \begin{aligned}
% \zeta_1(x) = c_1 \zeta_{1,1}(x) + c_2 \zeta_{1,2}(x) + c_3 \tilde{\zeta}_{1,3}(x) + c_4 \tilde{\zeta}_{1,4}(x)
% \end{aligned}
% \end{equation}
% \textbf{Note:} we did not finish this computation, because we cannot impose initial conditions due to cancellations.

\subsection{Fourier transform Claudio's version}

Using the convention
\begin{equation}\label{eq:claudios-convention}
\zeta(s) := \int_{-\infty}^\infty \dd{x}\zeta(x)e^{-ixs}
\end{equation}
the, with the inverse defined as
\begin{equation*}
\zeta(x) = \int_{-\infty}^\infty \frac{\dd{s}}{2\pi}\zeta(s)e^{ixs},
\end{equation*}
we have the following solution in Fourier-space
\begin{equation}
\begin{aligned}
\zeta(s) &= c_1\frac{1}{s}P_1^{i\lambda}(1+s) + c_2\frac{1}{s}Q_1^{i\lambda}(1+s)\\
&= c_1P_1^{i\lambda}(1+s) + c_2P_1^{-i\lambda}(1+s)\\
&= c_1\qty[\qty(s^{-i\lambda/2}(2+s)^{i\lambda/2}) + (1- i\lambda)(s^{-1-i\lambda/2}(2+s)^{i\lambda/2})] + c_2\qty[\lambda\to-\lambda],
\end{aligned}
\end{equation}
where is useful to define
\begin{equation}
\begin{aligned}
\tilde{\zeta}_1(s) &:= s^{-i\lambda/2}(2+s)^{i\lambda/2},\\
\tilde{\zeta}_2(s) &:= s^{-1-i\lambda/2}(2+s)^{i\lambda/2} = s^{-1}\tilde{\zeta}_1(s).
\end{aligned}
\end{equation}
Using \texttt{Mathematica} we can compute the Fourier inverse of the simplest functions:
\begin{equation}
f(x) := \mathcal{F}^{-1}\qty(s^{-i\alpha}) \propto  \frac{1}{x}(x-\abs{x}){\abs{x}}^{-1+i\alpha}
\end{equation}
and
\begin{equation}
g(x) := \mathcal{F}^{-1}\qty((2+s)^{i\alpha}) \propto e^{2i\abs{x}}(x - \abs{x}){\abs{x}}^{-2-i\alpha}.
\end{equation}

\subsubsection{Convolution: first try}

To compute the inverse of $\tilde{\zeta}_1(s)$ we will use convolution, defined as
\begin{equation*}
f\circ g = \int_{-\infty}^\infty f(u)g(x-u)\dd{u},
\end{equation*}
in our case
\begin{equation}
\begin{aligned}
\mathcal{F}^{-1}\qty[\tilde{\zeta}_1(s)](x) &=  \int_{-\infty}^\infty f(u)g(x-u)\dd{u}\\
&\propto \int_{-\infty}^\infty \frac{1}{u}(u-\abs{u}){\abs{u}}^{-1+i\alpha} e^{2i\abs{x-u}}(x-u - \abs{x-u}){\abs{x-u}}^{-2-i\alpha}\dd{u}.
\end{aligned}
\end{equation}
Note the important fact that $f(u)g(x-u)$ is equal to zero when
\begin{equation*}
u \geq 0 \qquad \text{or} \qquad x-u \geq 0
\end{equation*}
therefore we can restrict the integral range to $x < u < 0$. Considering this restriction we can rewrite the integral as
\begin{equation}
\begin{aligned}
\mathcal{F}^{-1}\qty[\tilde{\zeta}_1(s)](x) &\propto \int_x^0 2 (-u)^{-1 + i\alpha} e^{-2i(x-u)}2(x-u)(-(x-u))^{-2 - i\alpha}\dd{u}\\
& \propto -4\int_x^0 u^{-1 + i\alpha}e^{-2i(x-u)}(x-u)^{-1 - i\alpha}\dd{u}.
\end{aligned}
\end{equation}
\textbf{Note:} we did not finish this computation. See \eqref{subsec:Rewriting convolved functions} for the full computation.

\subsubsection{Convolution: second try}\label{subsec:Rewriting convolved functions}

We can write $f(x)$ and $g(x)$ in function of Heaviside functions
\begin{equation}
\begin{aligned}
f(x) &\propto -2(-1)^{i\alpha}\theta(-x)x^{-1+i\alpha}\\
g(x) &\propto 2(-1)^{-i\alpha}\theta(-x)e^{-2ix}x^{-1-i\alpha}.
\end{aligned}
\end{equation}
Now Mathematica is able to compute the convolution using the function \texttt{Convolve},
\begin{equation}
\tilde{\zeta}_1(s) = f\circ g \propto -\lambda \theta (-x) \, _1F_1(1-i \frac{\lambda}{2} ;2;-2 i x).
\end{equation}
For $\tilde{\zeta}_2(s) = s^{-1}\tilde{\zeta}_1(s)$ we define another sub-function,
\begin{equation*}
h(x) := \mathcal{F}^{-1}\qty[s^{-1}] = \frac{1}{2} i (1-i \lambda ) \text{sgn}(x).
\end{equation*}
First convolving $f(x)$ with $h(x)$, and this result convolved with $g(x)$
\begin{equation}
    \tilde{\zeta}_2(s) = g\circ(f\circ h) \propto (\lambda +i) \theta (-x) L_{\frac{i \lambda }{2}}(-2 i x).
\end{equation}
Finally
\begin{equation}
\begin{aligned}
\zeta(x) := \mathcal{F}^{-1}\qty[\zeta(s)] &= c_1\qty[\mathcal{F}^{-1}\qty[\tilde{\zeta}_1(s)] + \mathcal{F}^{-1}\qty[\tilde{\zeta}_2(s)]] + c_2\qty[\lambda \to -\lambda]\\
&=c_1\theta(-x)\qty[(\lambda + i)L_{i\lambda/2}(2-ix) - \lambda \,_1F_1\qty(1-i\frac{\lambda}{2};2;-2ix)]\\
&  \quad + c_2\theta(-x)\qty[\lambda \to -\lambda]
\end{aligned}
\end{equation}
that is exactly the same as \eqref{eq:sol-zeta1(x)_xpos}.

\section{A particular value of $\lambda$}

For some \textbf{unknown} reason the 4th order ODE for $\zeta_1(x)$ is \textit{simpler} when we take the \textbf{particular} value $\lambda = 2i$. Doing this
\begin{equation}
x^2\zeta_1^{(4)}(x) + 4x(1+ ix)\zeta_1^{(3)}(x) + 2x (5i-2x)\zeta_1''(x) - 2(i+2x)\zeta_1'(x) + 4\zeta_1(x)=0,
\end{equation}
that can be solved by \texttt{Mathematica}
...
It is important to note that replacing $\lambda = 2i$ on \eqref{eq:sol-zeta1(x)_xpos}
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
therefore we conclude that our solutions specified in \eqref{eq:sol-zeta1(x)_xpos} are correct!

Also, we can solve the ODE of $\zeta_2(x)$, given by
\begin{equation}
x^2\zeta_2 ^{(4)}(x) +(-4ix^2+4x) \zeta_2 ^{(3)}(x)+ (-4x^2-10ix) \zeta_2 ''(x)+(-4 x+2 i) \zeta_2 '(x)-\lambda ^2 \zeta_2 (x)=0,
\end{equation}
where the solution is given by
\begin{equation}\label{eq:zeta_2(x)-sol-lambda=2i}
\begin{aligned}
\zeta_2(x) &= c_1(1 + 2ix)\\
&\quad + c_2e^{2ix}\\
&\quad + c_3\frac{e^{2 i x}}{x}\qty[4 e^{-2 i x}x (1+2 i x) \text{Ei}(2 i x) -3 x + 4 i ]\\
&\quad + c_4 \frac{e^{2 i x}}{x\, \text{sgn}(x-1)} \qty[-(2+2 i) e^{-2 i x} x (1 + 2ix) \text{Ei}(2 i x)+ ix - 2i + 2)].
\end{aligned}
\end{equation}
Note the similarities between \eqref{eq:zeta_1(x)-sol-lambda=2i} and \eqref{eq:zeta_2(x)-sol-lambda=2i}.



\subsection{From basis}

Let us analyze the complete 1st order ODE system:
\begin{equation}
\begin{aligned}
    \zeta_1'= &~{} \frac{i\lambda }{2x^2} \left[ \left(ix- 1\right)\psi_1 +\left(ix+ 1\right)e^{-2i x}\psi_2 \right], \\
    \zeta_2'=&~{}  \frac{i\lambda}{2x^2} \left[ \left(ix-1\right)e^{2i x}\psi_1 + \left(ix+ 1\right)\psi_2 \right], \\
...
\end{aligned}
\end{equation}
that replacing $\lambda = 2i$ we would obtain
\begin{equation}
\begin{aligned}
    \zeta_1'(x) = &~{} -\frac{1}{x^2} \left[ \left(ix- 1\right)\psi_1(x) +\left(ix+ 1\right)e^{-2i x}\psi_2(x) \right], \\
    \zeta_2'(x) =&~{}  -\frac{1}{x^2} \left[ \left(ix-1\right)e^{2i x}\psi_1(x) + \left(ix+ 1\right)\psi_2(x) \right], \\
    \psi_1'(x) = &~{}  -\frac{1}{x^2} \left[- \left(ix + 1\right)\zeta_1(x) +\left(ix + 1\right)e^{-2i x}\zeta_2(x) \right], \\
    \psi_2'(x) =&~{}  -\frac{1}{x^2} \left[ \left(ix - 1\right)e^{2i x}\zeta_1(x) - \left(ix - 1\right)\zeta_2(x) \right].
\end{aligned}
\end{equation}
Until now, \today, I'm not sure why this system is simpler than the one with an arbitrary $\lambda$.


\section{Original 4th order ODE}

Actually, when we do the change of variable $x = e^{-t}$, from \eqref{eq: zeta_1-4order} we obtain
\begin{equation}
x^2\qty[x^2\zeta_1^{(4)}(x) + 4x(1+ ix)\zeta_1^{(3)}(x) + 2x (5i-2x)\zeta_1''(x) - 2(i+2x)\zeta_1'(x) - \lambda^2\zeta_1(x)]=0,
\end{equation}
where the ODE inside the square parenthesis is the ODE that we have been working on, Equation \eqref{eq:zeta_1(x)}.

Afterward, taking into account this overall quadratic term, we will have a new ODE
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
\subsection{Same $\lambda$ particular value}

For a moment let us consider $\lambda = 2i$ and Fourier transform our new ODE (using Claudio's convention, Equation \eqref{eq:claudios-convention})
\begin{equation}
\left(-s^4-4 s^3-4 s^2\right) F''(s)+\left(-4 s^3-14 s^2-12 s\right) F'(s)-2 s F(s)=a+b s
\end{equation}
that has the solution
\begin{equation}
\begin{aligned}
F(s) &= c_1\frac{1}{s^2 (s+2)} + c_2\frac{s+3}{ s+2} + b\qty[\frac{1}{2s} - \frac{1}{s^2}]\\
&\quad + a\Bigg[\frac{1}{s^2 (s+2)}-\frac{1}{6}\frac{1}{s+2} + \frac{1}{6}\frac{1}{ s (s+2)}\\
&\qquad\quad -\frac{1}{3}\frac{\log (s+2)}{s^2 (s+2)} -\frac{1}{4}\frac{\log (-s)}{ s+2} -\frac{1}{12}\frac{s \log (-s)}{s+2} + \frac{1}{4}\frac{\log (s+2)}{s+2}+\frac{1}{12}\frac{s \log (s+2)}{s+2}\Bigg].
\end{aligned}
\end{equation}
Until now, we are able to compute the inverse Fourier transform of all of functions, except the term
\begin{equation*}
F(s) \supset \frac{\log(s+2)}{s^2(s+2)}.
\end{equation*}
After computing the complete inverse Fourier transform of $F(s)$ we must re-obtain \eqref{eq:zeta_1(x)-sol-lambda=2i}. However, I think that there is a problem with that solution, because $\zeta_{1,3}$ and $\zeta_{1,4}$ are not linear independent.




\section{Using known solutions}

I think we should use the solutions we have already gotten, Equations \eqref{eq:sol-zeta1(x)_xpos}, how?? I dont know yet...

Below some possibilities:
\begin{enumerate}
    \item I think we can propose our new solutions like
    \begin{equation*}
    \begin{aligned}
        \zeta_{1,3}(x) &= u_3(x)\zeta_{1,1}(x)\\
        \zeta_{1,4}(x) &= u_4(x)\zeta_{1,2}(x)
    \end{aligned}
    \end{equation*}
    and replace on the ODE and solve for $u(x)$. I think it could cancel some terms and, get a lower order ODE??

    \item And if we do not consider the relation between $a$ and $b$ in the Laplace transform for the new solutions? We would have two additional l.i. solutions, particular solutions
\end{enumerate}
